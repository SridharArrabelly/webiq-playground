"""Tests for the SDK backend (offline — the SDK client is faked, no network).
...
def test_sdk_backend_search(monkeypatch, feature, has_content_format):
    ...
    result = b.search(feature, "deadline", site="wikipedia.org", max_results=3, region="ZA")

    ns = getattr(fake, feature)
    assert ns.called_query == "deadline site:wikipedia.org"
